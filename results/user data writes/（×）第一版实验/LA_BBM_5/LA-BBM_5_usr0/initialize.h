...
struct ssd_info{ 
	...
	double write_avg;                   //Record the time to calculate the average response time for the write request
	double read_avg;                    //Record the time to calculate the average response time for the read request
	...
	char zyfilename[50];
	char bsh_wl_skip_filename[50];
	char bsh_bad_block[50];
...
struct page_info{                      //lpn records the physical page stored in the logical page, when the logical page is valid, valid_state>0, free_state>0
	int fail;							// 1 for fail, 0 for healthy
	int valid_state;                   //indicate the page is valid or invalid
	int free_state;                    // just boolean val, indicates whether the page is free or not
	unsigned int lpn;                 
