...
int get_requests(struct ssd_info *ssd)
{
	...
	while (TRUE)
	{
		...
		{
			while (ope != READ) // seek for READ op, and convert it to WRITE op
			{
			...
			ope = WRITE; //  convert into WRITE op
		}
	...
	time_t += ssd->simulated_start_time;
...
//	request1->request_read_num = ssd->request_lz_count;
	...
	if (ssd->request_lz_count % 10000 == 0) printf("request:%I64u\n", ssd->request_lz_count);
	//printf("%d\n", ssd->request_queue_length);
