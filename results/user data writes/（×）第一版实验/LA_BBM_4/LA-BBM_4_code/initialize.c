...
struct ssd_info *initiation(struct ssd_info *ssd)
{
	...
	//printf("\n");
	if ((err = fopen_s(&ssd->bshfile, ssd->bsh_wl_skip_filename, "w")) != 0)
	{
		printf("the bsh_wl_skipped_rec file can't open\n");
		return NULL;
	}
	if ((err = fopen_s(&ssd->bsh_bad_block_file, ssd->bsh_bad_block, "w")) != 0)
	{
		printf("the bsh_bad_block.txt file can't open\n");
		return NULL;
	}
...
struct dram_info * initialize_dram(struct ssd_info * ssd)
{
	...
	if (ssd->parameter->allocation_scheme == DYNAMIC_ALLOCATION) // jump here allocation_scheme = 0
	{
