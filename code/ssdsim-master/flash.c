...
/**************************************************************************************************
 *函数的功能是寻找活跃块，每个plane中都只有一个活跃块，只有在这个活跃块中才能进行操作
 *若当前活跃块中已无空闲页可用，即此块空间已耗尽，则令此plane中下一个block作为活跃块以备后续使用
 *若活跃块计数已超界限（ssd->parameter->block_plane），则表明此plane中所有block已耗尽，无block可作活跃块
 *************************************************************************************************/
...
/*************************************************
 *这个函数的功能就是模拟一个实实在在的写操作（写page）
 *即更改这个page的相关参数，以及整个ssd的统计参数
 **************************************************/
