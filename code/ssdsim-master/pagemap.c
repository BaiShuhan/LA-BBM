...
struct ssd_info *pre_process_page(struct ssd_info *ssd)
{
    ...
    unsigned int largest_lsn,sub_size,ppn,add_size=0;  //最大逻辑扇区号，子页操作长度，物理页号，该IO已处理完毕的长度
    unsigned int i=0,j,k;
    int map_entry_new,map_entry_old,modify;
    int flag=0;
    char buffer_request[200];
    struct local *location;  //物理页号相关的结构体
    int64_t time;
	...
	//full_page初始化，full_page十六进制数从低位其，若page中子页数量为n,则将第0-n-1位二进制位置为1，其余n-31位置为0
    //用于操作后子页映射有效位的设置
    ...
    while(fgets(buffer_request,200,ssd->tracefile))  //逐行从tracefile文件中每次读取200个字符，直到整个trace读完为止
    {
        ...
        add_size=0;                                                     /*add_size是此IO操作已经预处理的大小*/

        if(ope==1)                                                      /*这里只是读请求的预处理，需要提前将相应位置的信息进行相应修改*/
        {
            while(add_size<size)  //已经处理的IO操作长度<此IO操作长度时，表示该IO还未处理完毕，需继续处理
            {				
                lsn=lsn%largest_lsn;                                    /*防止获得的lsn比最大的lsn还大*/		
                sub_size=ssd->parameter->subpage_page-(lsn%ssd->parameter->subpage_page);  //sub_size即lsn所在的page中实际IO需要操作的长度  //(lsn%ssd->parameter->subpage_page)定位此IO操作是从此page中的第几个subpage开始的，则此page中从此subpage开始之后的subpage都是需要被读取的，也就是此处求得的sub_size		
                //由此可以看出，sub_size是在lsn所在的这一个page中所需读取的长度，而一个IO操作可能跨越多个page,则将这一个IO操作按page界限划分为多个子IO操作
				if(add_size+sub_size>=size)                             /*只有当一个IO操作的大小小于一个page的大小时或者是处理一个IO操作的最后一个page所在的子IO操作时（若最后一个子IO操作未读完整个page，仅读到page中间时）会出现这种情况*/
                {		
				...
				//虽然映射表记录的是lpn->ppn的映射，但块IO接口实际传输单位是sector，IO操作时也是以lsn来进行的
				//则一个page中有subpage写入时则会存在lpn->ppn映射,但根据IO操作具体信息的不同，该page中各subpage子页映射有效位的设置情况是不同的
				//故需要根据ssd->dram->map->map_entry[lpn].state取值来进行后续映射信息设置
                /*******************************************************************************************************
                 *利用逻辑扇区号lsn计算出逻辑页号lpn
                 *判断dram中映射表map中lpn位置的状态
                 *A，这个状态==0，表示以前没有写过，不具备映射，lpn对应物理页还未成功映射至内存lpn且数据仍未拷贝至buffer中，现在需要直接将sub_size大小的子页写进去
                 *B，这个状态>0，表示，以前有写过，具有映射，映射有效子页数据可直接从buffer中进行读取，但这需要进一步比较子页映射状态，因为新写的状态可以与以前的状态有重叠的子页的地方
                 ********************************************************************************************************/
                lpn=lsn/ssd->parameter->subpage_page;
                if(ssd->dram->map->map_entry[lpn].state==0)                 /*状态为0的情况*/
                {
                    /**************************************************************
                     *利用get_ppn_for_pre_process函数获得ppn，再得到location
                    ...
                    ssd->dram->map->map_entry[lpn].state=set_entry_state(ssd,lsn,sub_size);   //根据子IO操作的lsn,sub_size参数置对应子页的映射为有效
                    ...
                    ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].free_state=((~ssd->dram->map->map_entry[lpn].state)&full_page);  //state位有效为1，无效为0（从右到左的二进制位，对应的子页号从低到高，即最低位对应子页0）；free_state位free为1，used为0；此将映射有效子页（即要被读取的子页）置为used
                ...
                {
                    map_entry_new=set_entry_state(ssd,lsn,sub_size);      /*得到新的状态，并与原来的状态相或得到一个状态*/
                    map_entry_old=ssd->dram->map->map_entry[lpn].state;
                    modify=map_entry_new|map_entry_old;   //此子IO操作使映射有效的子页，以及，之前操作使映射有效的子页，modify后，这些子页均应依旧映射有效，故相或
                    ppn=ssd->dram->map->map_entry[lpn].pn;   //lpn之前已有映射，可以直接取ppn
                    location=find_location(ssd,ppn);
...
unsigned int get_ppn_for_pre_process(struct ssd_info *ssd,unsigned int lsn)     
{
    ...
    {
        if (ssd->parameter->dynamic_allocation==0)                      /*表示全动态方式下，也就是channel，chip，die，plane等都是动态分配*/
        {
            channel=ssd->token;  ////在动态分配中，为防止每次分配在第一个channel需要维持一个令牌，每次从令牌所指的位置开始分配
            ssd->token=(ssd->token+1)%ssd->parameter->channel_number;  //设置下一次动态分配的channel token
            chip=ssd->channel_head[channel].token;  //在动态分配中，为防止每次分配在第一个chip需要维持一个令牌，每次从令牌所指的位置开始分配
            ssd->channel_head[channel].token=(chip+1)%ssd->parameter->chip_channel[0];  //设置下一次动态分配的chip token
            die=ssd->channel_head[channel].chip_head[chip].token;  //在动态分配中，为防止每次分配在第一个die需要维持一个令牌，每次从令牌所指的位置开始分配
            ssd->channel_head[channel].chip_head[chip].token=(die+1)%ssd->parameter->die_chip;  //设置下一次动态分配的die token
            plane=ssd->channel_head[channel].chip_head[chip].die_head[die].token;  //在动态分配中，为防止每次分配在第一个plane需要维持一个令牌，每次从令牌所指的位置开始分配
            ssd->channel_head[channel].chip_head[chip].die_head[die].token=(plane+1)%ssd->parameter->plane_die;  //设置下一次动态分配的plane token
        } 
