...
struct ssd_info *initiation(struct ssd_info *ssd)
{
    ...
    //初始化通道,initialize_channels()函数中包含下层结构的initialize_chip()函数,...,依次,channel->chip->die->plane->block->page各层结构均实现初始化
	...
	//实现outputfile与statisticfile中预备信息的显示输出
    printf("\n");
    ...
    while(fgets(buffer,300,fp))  //此时fp指针指向的是参数文件  //逐行读取参数文件中的数据到缓冲区buffer中，每次读取一行300字节后会执行循环体内的代码部分
    {
...
//初始化block，其中引用初始化page函数以实现对各block中各page的初始化
...
//初始化plane，其中引用初始化block函数以实现对各plane中各block的初始化
...
//初始化die，其中引用初始化plane函数以实现对各die中各plane的初始化
...
//初始化chip，其中引用初始化die函数以实现对各chip中各die的初始化
...
//初始化通道，其中引用初始化chip函数以实现对各通道中各chip的初始化
...
struct parameter_value *load_parameters(char parameter_file[30])
{
    ...
    while(fgets(buf,200,fp)){
        ...
        ptr=strchr(buf,'=');  //返回buf中国首次出现'='的位置
        if(!ptr) continue; 

        pre_eql = ptr - buf;  //buf指buf[]数组的首地址即下标0，则pre_eql代表buf中'='之前的字符的数量
        next_eql = pre_eql + 1;  //next_eql代表buf中'='下一个字符的位置

        while(buf[pre_eql-1] == ' ') pre_eql--;  //若'='前面的位置的字符是空格，则一直向前推进直到pre_eql所指的位置的字符非空格，这种方式避免了诸如'chip number = **'这样的参数所带来的处理问题
        buf[pre_eql] = 0;  //令'='前面的非空格字符的后一个字符为0，如果'='之前没有空格则是将'='赋值为0
		//这样做的原因是因为后面使用strcmp()字符串比较函数时，只有在字符串中遇到'/0'才会停止比较
		//下面的if多分支结构的作用是负责在buf中逐一读取相关参数值到parameter_value中
