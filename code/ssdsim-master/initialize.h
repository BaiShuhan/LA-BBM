...
struct channel_info{
    ...
    int64_t next_state_predict_time;     //the predict time of next state, used to decide the state at the moment
...
struct chip_info{
    ...
    int next_state;
    int64_t current_time;               //记录该chip的当前时间
    int64_t next_state_predict_time;    //the predict time of next state, used to decide the state at the moment
...
struct blk_info{
    ...
    int last_write_page;               //记录最近一次写操作执行到的页数,-1表示该块没有一页被写过
