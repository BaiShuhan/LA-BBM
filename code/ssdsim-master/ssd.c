...
struct ssd_info *simulate(struct ssd_info *ssd)
{
    ...
    while(flag!=100)   //IO操作相关参数非法则get_requests()函数返回100    
    {

        flag=get_requests(ssd);  //IO操作是一个一个挂载到request_queue上的

        if(flag == 1)   //若一个IO操作挂载成功则执行下述程序
        {   
            //printf("once\n");
...
{  
    ...
    int flag = 1;
    long filepoint; 
    int64_t time_t = 0;  //time_t记录IO到达时间
    ...
    if(feof(ssd->tracefile))  //用于测试文件的当前状态是否为“文件结束”，若是，则返回1，否则返回0
        return 0; 

    filepoint = ftell(ssd->tracefile);	//用于定位当前文件指针位置，filepoint记录当前文件指针位置相对于文件首部位置的偏移字节数
     ...
     *large_lsn: channel中总共有多少个subpage（即多少个sector）可以给用户使用。
	 *overprovide系数：SSD中并不是所有的空间都可以给用户使用，比如32G的SSD可能有10%的空间保留下来留作他用。
     ***********************************************************************************************************/
    ...
    lsn = lsn%large_lsn;  //channel中具体的逻辑扇区号



	/******************************************************************************************************
	*下面设置系统时间ssd->current_time：主要是判断下一状态预计时间与IO到达时间之间的关系
	*1. 下一状态预计时间 < IO到达时间，IO被阻塞，需回滚，无IO挂载到request_queue
	*2. 下一状态预计时间 > IO到达时间，IO可被处理
	  *2.1. 若下一状态预计时间为极限值，则肯定可以挂载
	  *2.2. 若下一状态预计时间不为极限值，则需进一步判断是否能正常挂载到request_queue，即判断是否队列满
	******************************************************************************************************/
    nearest_event_time=find_nearest_event(ssd);  //返回当前所有子请求中最早到达的下一个状态时间
    if (nearest_event_time==MAX_INT64)  //MAX_INT64代表转换到下一状态的时间极限值
    {
        ssd->current_time=time_t;  //若下一状态预计时间等于极限值，表明无法获取合理的下一状态预计时间
		                           //但IO请求是可以被处理的，只是下一状态预计时间为最大极限值，IO会一直被阻塞
		                           //需要更新系统事件为IO到达时间表示IO请求已经开始处理
    ...
    else  //小于极限值则表示nearest_event_time是合理且可用的，表明下一个状态是可以到达的
    {   
        if(nearest_event_time<time_t)  //若下一状态预计到达时间<IO到达时间，则说明此IO处于阻塞状态
        {  //获取的事件预计时间（即下一状态预计时间）与实际的IO参数不符合，需要执行回滚
		   //回滚即当程序或数据处理错误，将程序或数据恢复到上一次正确状态
            /*******************************************************************************
             ...
             **********************************************************************************/
            fseek(ssd->tracefile,filepoint,0);   //回滚至filepoint偏移处
            if(ssd->current_time<=nearest_event_time)  //若下一状态可到达
                ssd->current_time=nearest_event_time;  //更新系统时间为下一状态预计时间
            return -1;  //因为回滚，无IO操作挂载至request_queue
        }
        else  //若下一状态预计时间>IO到达时间，此时需要进一步确认IO能否正常插入请求队列
        {
            if (ssd->request_queue_length>=ssd->parameter->queue_length)  //请求队列满则回滚，无IO挂载，退出函数
            {
                fseek(ssd->tracefile,filepoint,0);
                ssd->current_time=nearest_event_time;  //更新系统时间为下一状态预计时间，因为“队列满回滚”这个下一状态已到达
                return -1;
            } 
            else
            {
                ssd->current_time=time_t;   //若可正常插入队列则更新系统时间为IO到达时间
            }
        }
    }
 ...
 *寻找所有子请求中最早到达的下个状态时间,首先看请求的下一个状态时间，如果请求的下个状态时间小于等于当前时间，
 *说明请求被阻塞，需要查看channel或者对应die的下一状态时间。
 *Int64是无符号 64 位整数数据类型，最大值为0x7fffffffffffffffll。存储空间占 8 字节。
 *channel,cihp是事件向前推进的关键因素，三种情况可以使事件继续向前推进:channel，chip分别回到idle状态，chip中的
 *读数据准备好了
 ***********************************************************************************************************/
int64_t find_nearest_event(struct ssd_info *ssd) 
{
    unsigned int i,j;
    int64_t time=MAX_INT64;  //全部初始化为极限值0x7fffffffffffffffll（无符号Int64的最大值）
    int64_t time1=MAX_INT64;  //记录channel的下一状态预计时间最小值
    int64_t time2=MAX_INT64;  //记录chip的下一状态预计时间最小值
        ...
        if (ssd->channel_head[i].next_state==CHANNEL_IDLE)  
            if(time1>ssd->channel_head[i].next_state_predict_time)  
                if (ssd->channel_head[i].next_state_predict_time>ssd->current_time)  
                    time1=ssd->channel_head[i].next_state_predict_time;  //对于遍历的每一个channel，以此time1来比较查询其他channel的下一状态预计时间，并从中获得最小值
            ...
            if ((ssd->channel_head[i].chip_head[j].next_state==CHIP_IDLE)||(ssd->channel_head[i].chip_head[j].next_state==CHIP_DATA_TRANSFER))
                if(time2>ssd->channel_head[i].chip_head[j].next_state_predict_time)
                    if (ssd->channel_head[i].chip_head[j].next_state_predict_time>ssd->current_time)    
                        time2=ssd->channel_head[i].chip_head[j].next_state_predict_time;	//对于遍历的每一个chip，以此time1来比较查询其他chip的下一状态预计时间，并从中获得最小值
        }   
    } 

    /*****************************************************************************************************
     *time为所有 A.下一状态为CHANNEL_IDLE，且下一状态预计时间大于ssd当前时间的，CHANNEL的下一状态预计时间
     *           B.下一状态为CHIP_IDLE，且下一状态预计时间大于ssd当前时间的，CHIP的下一状态预计时间
     *		     C.下一状态为CHIP_DATA_TRANSFER，且下一状态预计时间大于ssd当前时间的，CHIP的下一状态预计时间
     *中的最小值；
	 *CHIP_DATA_TRANSFER读准备好状态，数据已从介质传到了register，下一状态是从register传往buffer；
                ...
                for (k=0;k<ssd->parameter->die_chip;k++)
                    ...
                    {  
                        ...
                        {  
                            if (flag>=threshould)
                            {
                                break;   //筛选出那些失效页数量已经大于阈值的plane,这些plane无需再额外置page失效
                            }
